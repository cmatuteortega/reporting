...
time_data <- fread('time_converter.csv')
...
# OCUPACION TURNO
turno_sql <- 'SELECT * FROM digital_business_reporting.ocupacion_turnos'
turno <- data.table(bq_table_download(bq_project_query(projectid,turno_sql)))
turno <- turno[service_date == as.POSIXct(mesdia)] 
...
tc_mad <- tc_mad[`Fecha Entrega`>=mesdialy]
mesdialyotro <- mesdialy+31
tc_mad <- tc_mad[`Fecha Entrega`<=mesdialyotro]
...
total_mo <- data.table('Total MO' = sum(resumen_inicial$venta_total), 'vs Previsión' = paste0(round(100*(1-(sum(master_pred$pred_facturacion)/1000000)/sum(resumen_inicial$venta_total)),0),"%"), titulo =  paste0(round(100*(sum(resumen_inicial$venta_total)/(sum(sample[mes == mesdialy]$venta,na.rm=T)/1000000)-1),0),"%") )
...
# ---------------------------------- PLOT  -------------------------------------------------
venta_prev_tabla_plot <- venta_prev_tabla_1[c(2,1,3)]

kable(venta_prev_tabla_plot,align = 'c', format='latex') %>%
  kable_paper("striped", full_width = F)%>% 
  kable_styling(latex_options ="scale_down")%>%
  row_spec(0, bold = TRUE)%>%
  row_spec(1, bold=T, font_size = 12,color = "white", background='#13C745')%>%
  row_spec(3, italic = T, color = "black", background='#8aedc1')
...
# ---------------------------------- PLOT  -------------------------------------------------
venta_prev_tabla_plot <- venta_prev_tabla[c(2,1,3)]
...
ocupacion_despues <- sum(sample[hive_code==hive & mes == mesdia]$col_vendidas)/sum(previs_master[hive_code==hive & month(date)==mesnum]$expected_columns)

ocupacion_antes <- sum(sample[hive_code==hive & mes == mesdia]$col_vendidas)/sum(previs_master[hive_code==hive & month(date)==mesnum]$original_columns)

diff <- round(100*(ocupacion_despues-ocupacion_antes),1)
...
ajustes <- data.table('Subidas Flash' = c(sum(previs[VAR>0]$VAR),paste0(round(sum(previs[VAR>0]$VAR)*preciomedio/1000,1)," mil €")), 
                      'Bajadas Flash' = c(sum(previs[VAR<0]$VAR),if (diff > 0) {paste0("+",diff,"% ocupación")} else {"-"} ))

ajustes[,'':=c('Columnas','Impacto')]
...
kable(error_box,align = 'l', format='latex',linesep = "") %>%
  kable_paper("striped", full_width = F)%>% 
  kable_styling(font_size = 7)%>%
  column_spec(ncol(ajustes), bold = T)%>%
  row_spec(0,bold=T)
```
...
- *Ocupación*: `r venta_prev[2]$Ocupación`, donde el TM ha tenido un `r trunc(turno[colmena==hive_code & turno == 'Mañana']$ocupacion*100,0)`% y el TT un `r trunc(turno[colmena==hive_code & turno == 'Tarde']$ocupacion*100,0)`%
...
- *Previsiones*: `r if (error[Modo=='Oferta']$Total[1]<obj_error[1] & error[Modo=='Unidades']$Total[1]<obj_error[2]) {'Ambos errores están **OK** con el objetivo de Negocio'} else if (error[Modo=='Oferta']$Total[1]<obj_error[1]) {'La previsión de uds/columna está KO'} else if (error[Modo=='Unidades']$Total[1]<obj_error[1]) {'La previsión de la oferta está KO'} else {'Ambas previsiones están **KO** con el objetivo de Negocio.'}`.
  - Error de la oferta: `r error_box[1,3]` por quedarse `r error[Modo=='Oferta']$Tipo[1]`.
  - Error de unidades por columna: `r error_box[2,3]` por quedarse `r error[Modo=='Unidades']$Tipo[1]`.
...
kable(venta_prev_tabla_plot,align = 'c', format='latex') %>%
  ...
  kable_styling(latex_options="scale_down")%>%
  row_spec(0, bold = TRUE)%>%
  row_spec(1, bold=T, font_size = 12,color = "white", background='#006AE7')%>%
  row_spec(3, italic = T, color = "black", background='#9dc7fa')
...
kable(venta_prev_tabla_plot,align = 'c', format='latex') %>%
  ...
  kable_styling(latex_options="scale_down")%>%
  row_spec(0, bold = TRUE)%>%
  row_spec(1, bold=T, font_size = 12,color = "white", background='#F0371E')%>%
  row_spec(3, italic = T, color = "black", background='#fc8f81')
...
kable(venta_prev_tabla_plot,align = 'c', format='latex') %>%
  ...
  kable_styling(latex_options="scale_down")%>%
  row_spec(0, bold = TRUE)%>%
  row_spec(1, bold=T, font_size = 12,color = "white", background='#F0BB00')%>%
  row_spec(3, italic = T, color = "black", background='#fadf7d')
...
kable(venta_prev_tabla_plot,align = 'c', format='latex') %>%
  ...
  kable_styling(latex_options="scale_down")%>%
  row_spec(0, bold = TRUE)%>%
  row_spec(1, bold=T, font_size = 12,color = "white", background='#BF56E4')%>%
  row_spec(3, italic = T, color = "black", background='#d6a5e8')
...
- *Ocupación*: `r venta_prev[2]$Ocupación` con panales, mientras que el TM en colmena ha tenido un `r trunc(turno[colmena==hive_code & turno == 'Mañana']$ocupacion*100,0)`% y el TT un `r trunc(turno[colmena==hive_code & turno == 'Tarde']$ocupacion*100,0)`%
...
kable(venta_prev_tabla_plot,align = 'c', format='latex') %>%
  ...
  kable_styling(latex_options="scale_down")%>%
  row_spec(0, bold = TRUE)%>%
  row_spec(1, bold=T, font_size = 12,color = "white", background='#FFB5D4')%>%
  row_spec(3, italic = T, color = "black", background='#fad2e3')
...
mad_mult[,fecha_comparable:=first_service-364]
...
kable(mult_final,align = 'c', format='latex',linesep = "") %>%
  kable_paper("striped", full_width = F)%>% 
  kable_styling(font_size = 7)%>%
  row_spec(length(mult_final$Area),font_size = 10,bold=T,color = "black", background='#fad2e3')
...
- *Previsiones*:
  - Error de unidades por columna: `r error_box[1,3]` por quedarse `r error[Modo=='Unidades']$Tipo[1]`.
...
ggplot(tiendas_base, aes(x = month, y = peso_venta))+
  ...
  geom_text(aes(x = tail(month, 1), y = tail(peso_venta,1), label = scales::percent(tail(peso_venta,1))),vjust = -1, hjust = 1, color = "darkgray", size = 6)+scale_x_date(date_labels = "%m-%y") 
...
Hay un total de `r length(matriz_base[ocupacion_nsd > 0.6 & ddc_nsd < 1.5]$code)` NSD de `r length(matriz_base$code)` con una oferta bien dimensionada. La media de días de cola han sido `r round(mean(matriz_base$ddc_nsd,na.rm=T),1)`.
...
\centering
\small \textit{¿Dudas?} $\rightarrow$ #digital_business
